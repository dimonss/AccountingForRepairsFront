...
```
src/components/
├── PhotoUpload.tsx          # Компонент загрузки (обновлен)
├── PhotoUpload.css          # Стили загрузки (обновлены)
├── CameraCapture.tsx        # 🆕 Компонент съемки
├── CameraCapture.css        # 🆕 Стили камеры
...
### Для продакшена:
1. **Серверная загрузка**: Реализовать загрузку на сервер
2. **Сжатие изображений**: Автоматическое сжатие перед загрузкой
3. **Форматы**: Поддержка WebP для оптимизации
4. **Кэширование**: Кэширование миниатюр
5. **Безопасность**: Валидация типов файлов на сервере
